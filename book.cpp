// Format from HG Muller's documentation http://hgm.nubati.net/book_format.html

/*
book file source:

Human.bin, Titans.bin - Deep Saros engine https://digilander.libero.it/taioscacchi/programmi/saros-page.html
Book.bin - Ed Schröder's million base https://web.archive.org/web/20180104204744/http://top-5000.nl/pgn.htm, filtered with SCID, process from video https://www.youtube.com/watch?v=rskcUNoirPU
...
white pawn    1 -> PAWN = 0
white knight  3 -> KNIGHT = 1
white bishop  5 -> BISHOP = 2
white rook    7 -> ROOK = 3
white queen   9 -> QUEEN = 4
white king   11 -> KING = 5
black pawn    0 -> PAWN + 6 = 6
black knight  2 -> KNIGHT + 6 = 7
black bishop  4 -> BISHOP + 6 = 8
black rook    6 -> ROOK + 6 = 9
black queen   8 -> QUEEN + 6 = 10
black king   10 -> KING + 6 = 11
*/

enum RANDOM_OFFSETS { PIECE = 0, CASTLE = 768, EP = 772, TURN = 780 };

const int POLYGLOT_PIECE_TYPE[12] = { 1, 3, 5, 7, 9, 11, 0, 2, 4, 6, 8, 10 };
const int POLYGLOT_SQUARES[64][2] = {
...
ull book::gen_polyglot_key(bitboard::Position &board) {
	...
	for (int i = 0; i < 64; i++) {
		if (board.mailbox[i] == -1)
			continue;
		ull currentPiece = 64 * POLYGLOT_PIECE_TYPE[board.mailbox[i]] + 8 * POLYGLOT_SQUARES[i][1] + POLYGLOT_SQUARES[i][0];
		pieces ^= RANDOM[PIECE + currentPiece];
	}

	ull castle = 0;
	if (board.castle[WHITE_SHORT])
		castle ^= RANDOM[CASTLE + 0];
	if (board.castle[WHITE_LONG])
		castle ^= RANDOM[CASTLE + 1];
	if (board.castle[BLACK_SHORT])
		castle ^= RANDOM[CASTLE + 2];
...
int convert_polyglot_move(bitboard::Position &board, int move) {
	...
	int fromRow = (move & 0b111000000000) >> 9;
	int promote = (move & 0b111000000000000) >> 12;

	int dest = (destRow * 8 + (7 - destFile));
	int from = (fromRow * 8 + (7 - fromFile));

	// detect castling

	if (from == E1 && board.mailbox[dest] == KING) {
		if (dest == H1)
			return NEW_MOVE(E1, G1, WHITE_SHORT, 0, 0);
		if (dest == A1)
			return NEW_MOVE(E1, C1, WHITE_LONG, 0, 0);
	}
	if (from == E8 && board.mailbox[dest] == KING + 6) {
		if (dest == H8)
			return NEW_MOVE(E8, G8, BLACK_SHORT, 0, 0);
		if (dest == A8)
			return NEW_MOVE(E8, C8, BLACK_LONG, 0, 0);
	}

	return move::uci_to_move(board, from, dest, promote);
}

// CREDIT: Fabien Letouzey's Fruit Engine, version 2.1, src found at https://arctrix.com/nas/chess/fruit
// Code from here to end of file copied from Fruit with minor adjustments
...
void book::book_open(const char file_name[]) {
	book_init();
...
static ull read_integer(FILE *file, int size) {
	ull n;
	int b;
...
static void read_entry(Entry *entry, int n) {
	assert(entry != NULL);
...
int book::book_move(bitboard::Position &board) {
	...
	if (BookFile != NULL && BookSize != 0) {
		...
		for (int pos = find_pos(key); pos < BookSize; pos++) {
		...
		if (best_move != -1)
			return convert_polyglot_move(board, best_move);
	}
